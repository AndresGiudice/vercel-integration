...
export default function BolsasConManijaFantasia({ isConnected }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const handleAddToCart = () => {
    bags.forEach((bag, index) => {
      if (quantities[index] > 0) {
        const prefix = (bag.additionalDescription === "Surtido" && (bag.description.startsWith("G3") || bag.description.startsWith("G5"))) ? "" : "G";
        addToCart(bag.systemCode, quantities[index], `${prefix}${bag.description} ${additionalDescription}`, bag.list2);
      }
    });
      ...
      {bags.map((bag, index) => (
        <div key={bag.systemCode}>
          <div className="flex justify-center mb-2">
            <p className="text-gray-700 text-base mt-2">G{bag.description} - Precio x100: <span className="font-bold">${Math.round((bag.list2 * 0.9 * 0.95) / 1.105)}</span></p>
          </div>
