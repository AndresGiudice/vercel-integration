...
export default function BolsasFastFoodColorL4({ isConnected }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const handleAddToCart = (systemCode: string, description: string, additionalDescription: string = '') => {
    ...
    if (bag) {
      addToCart(systemCode, quantities[systemCode], `${cleanedDescription} ${cleanedAdditionalDescription}`, bag.list2);
    }
    setQuantities((prevQuantities) => ({
      ...prevQuantities,
      [systemCode]: 0,
                ...
                >
                 ...
                 <div className="container mx-auto p-2">
                  ...
                  <div className="flex justify-center mb-2">
                    <p className="text-gray-700 text-lg"> Precio x100: <span className="font-bold">${Math.round(calculateDiscountedPrice((bag.list2 * 0.9 * 0.95) / 1.105))}</span></p>
                  </div>
