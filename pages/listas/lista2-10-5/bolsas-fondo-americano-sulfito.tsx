...
export default function BolsasConManijaKraft({ isConnected }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const handleAddToCart = (systemCode: string, description: string, list2: number) => {
    ...
    addToCart(systemCode, quantities[systemCode], cleanedDescription, list2);
    setQuantities((prevQuantities) => ({
      ...prevQuantities,
      [systemCode]: 0,
    }));
              ...
              {bags.map((bag, index) => (
                ...
                >
                  ...
                  <div className="flex justify-center mb-2">
                    <p className="text-gray-700 text-lg"> Precio x1000: <span className="font-bold">${Math.round((bag.list2 * 0.9 * 0.95) / 1.105)}</span></p>
                  </div>
