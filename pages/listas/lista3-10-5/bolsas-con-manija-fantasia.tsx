  ...
  const handleAddToCart = () => {
    bags.forEach((bag, index) => {
      if (quantities[index] > 0) {
        ...
        addToCart(bag.systemCode, quantities[index], `${prefix}${bag.description} ${additionalDescription}`, bag.list3);
      }
    });
      ...
      {bags.map((bag, index) => (
        <div key={bag.systemCode}>
          <div className="flex justify-center mb-2">
            <p className="text-gray-700 text-base mt-2">G{bag.description} - Precio x100: <span className="font-bold">${Math.round((bag.list3 * 0.9 * 0.95) / 1.105)}</span></p>
          </div>
