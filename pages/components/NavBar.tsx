...
function Example() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/?search=${encodeURIComponent(searchQuery.trim())}`); // Redirect to home with search query
    }
  };
  ...
  return (
    <header className="fixed top-0 left-0 right-0 bg-[#A6CE39] z-50">
      <nav aria-label="Global" className="relative mx-auto flex items-center justify-between h-14 lg:h-16 p-2 lg:px-8 z-50">
        ...
        <form onSubmit={handleSearch} className="hidden lg:flex lg:gap-x-4">
          <input
            type="text"
            placeholder="Buscar..."
            value={searchQuery}
