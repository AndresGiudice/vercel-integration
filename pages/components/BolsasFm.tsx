...
const BolsasFm = ({ isConnected }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const handleAddToCart = (systemCode: string, description: string, list4: number, list3: number, list2: number) => {
    const cleanedDescription = description.replace(/Bolsa Fast Food\s*/, '').replace(/(Kraft).*/, 'Kraft');
    const cleanedAdditionalDescription = ''; // Removed additionalDescription as it's not part of the expected signature
    ...
    addToCart(systemCode, quantities[systemCode], cleanedDescription, price);
      setQuantities((prevQuantities) => ({
        ...prevQuantities,
        [systemCode]: 0,
      }));
