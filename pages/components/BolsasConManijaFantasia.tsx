  ...
  const handleAddToCart = () => {
    bags.forEach((bag, index) => {
      if (quantities[index] > 0) {
        ...
        addToCart(bag.systemCode, quantities[index], `${prefix}${bag.description} ${additionalDescription}`, price);
      }
    });
      ...
      {bags.map((bag, index) => (
        <div key={bag.systemCode}>
          <div className="flex justify-center mb-2">
            <p className="text-gray-700 text-base mt-2">G{bag.description} - Precio x100:   <span className="font-bold">
                {(() => {
                  let finalPrice = 0;
